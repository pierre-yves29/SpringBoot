...
public class MovieDAOImpl implements MovieDAO {
    ...
    private static final String SELECT_ALL = "SELECT f.id AS id_film, f.titre AS titre_film, f.annee, f.duree, f.synopsis, g.titre AS titre_genre, p.nom, p.prenom FROM FILM AS f INNER JOIN GENRE AS g ON g.id = f.id_genre INNER JOIN PARTICIPANT AS p ON p.id = f.id_realisateur";
//    private static final String SELECT_ALL = "SELECT * FROM FILM AS f INNER JOIN GENRE AS g ON g.id = f.id_genre INNER JOIN PARTICIPANT AS p ON p.id = f.id_realisateur";
    private static final String SELECT_BY_ID = "SELECT f.id AS id_film, f.titre AS titre_film, f.annee, f.duree, f.synopsis, g.titre AS titre_genre, p.nom, p.prenom FROM FILM AS f INNER JOIN GENRE AS g ON g.id = f.id_genre INNER JOIN PARTICIPANT AS p ON p.id = f.id_realisateur WHERE f.id = :id";
...
//        movie.setId(movieDB.getId());
    ...
    class MovieRowMapper implements RowMapper<Movie> {
        ...
        public Movie mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            Movie movie = new Movie();
            movie.setId(rs.getLong("id_film"));
            movie.setTitle(rs.getString("titre_film"));//titre film
            movie.setYear(rs.getInt("annee"));
            movie.setDuration(rs.getInt("duree"));
            movie.setSynopsis(rs.getString("synopsis"));

            Genre genre = new Genre();
            genre.setTitle(rs.getString("titre_genre"));//titre genre
